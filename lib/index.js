...
class VesilD {
    ...
    async _updatePoints(redisClient, leaderboardKey, memberKey, patchData) {
        ...
        // by removing and adding we hopefully trigger the watch command
        await redisClient
            .multi()
            .zrem(ranksRedisKey, points)
            .zadd(ranksRedisKey, points, points)
            .exec();
        ...
        if (prevPoints) {
            ...
            while(!success) {
                const numberOfPrev = parseInt(await redisClient.zcount(leaderboardRedisKey, prevPoints, prevPoints));

                if (numberOfPrev === 0) {
                    success = await redisClient
                        .multi()
                        .zrem(ranksRedisKey, prevPoints)
                        .exec();
    ...
    async runServer() {
        ...
        app.delete(leaderboardPath + memberPath, async (req, res) => {
            ...
            redisClient.zrem(ranksRedisKey, leaderboardRedisKey);
            redisClient.srem(memberRedisKey);
